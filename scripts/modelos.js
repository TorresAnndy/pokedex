...
var PokedexCollection = Backbone.Collection.extend({
  ...
  obtenerPorTipo: function (tipo) {
    var self = this;
    this.reset();
    $.getJSON(URL_API_TIPO + tipo).done(function (data) {
      var pokemons = data.pokemon.slice(0, 10); // limitar a 10
      pokemons.forEach((p) => {
        var poke = new PokemonModel({ nombre: p.pokemon.name });

        // Espera a que los datos estén listos antes de agregarlo a la colección
        poke.on("datos:listos", function () {
          self.add(poke);
        });

        poke.obtenerDatos(); // Hace la petición a la API y llena imagen, tipo, etc.
      });
    });
  },
});
